...
export default function EditProperty() {
  ...
  const fetchProperty = async () => {
    try {
      ...
      // Map nulls to empty strings for form inputs
      const mapped = {};
      Object.keys(DEFAULTS).forEach((k) => {
        const v = data[k];
        mapped[k] = v === null || v === undefined ? DEFAULTS[k] : v;
      });
      setForm(mapped);
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.address.toString().trim()) { setError("Address is required."); return; }
    ...
    <div style={PAGE}>
      <div style={{ borderBottom: "1px solid #E5E7EB", background: "white", padding: "16px 24px" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          <button onClick={() => router.push(`/portfolio/${params.id}`)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, color: "#6366F1", fontSize: 14, fontWeight: 500 }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 20px" }}>
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 8, display: "flex", alignItems: "center", gap: 10 }}>
            <Edit2 size={24} color="#6366F1" /> Edit Property
          </h1>
          <p style={{ color: "#666", fontSize: 14 }}>{form.address || "Update property details"}</p>
        </div>
          ...
          <div style={CARD}>
            ...
            <div style={{ ...GRID2, marginBottom: 16 }}>
              <div>
                ...
                <input style={INPUT} value={form.postcode} onChange={(e) => set("postcode", e.target.value)} />
              </div>
              <div>
            ...
            <div style={GRID3}>
              ...
              <div>
                ...
                <input style={INPUT} type="number" value={form.estimated_value} onChange={(e) => set("estimated_value", e.target.value)} />
              </div>
              <div>
                <label style={LABEL}>Purchase Price (£)</label>
                <input style={INPUT} type="number" value={form.purchase_price} onChange={(e) => set("purchase_price", e.target.value)} />
              </div>
            </div>
          ...
          <div style={CARD}>
            ...
            <div style={{ ...GRID2, marginBottom: 16 }}>
              <div>
                ...
                <input style={INPUT} type="number" value={form.outstanding_balance} onChange={(e) => set("outstanding_balance", e.target.value)} />
              </div>
              <div>
                <label style={LABEL}>Interest Rate (%)</label>
                <input style={INPUT} type="number" step="0.01" value={form.interest_rate} onChange={(e) => set("interest_rate", e.target.value)} />
              </div>
            </div>
            <div style={{ ...GRID3, marginBottom: 16 }}>
              ...
              <div>
                ...
                <input style={INPUT} type="number" value={form.monthly_payment} onChange={(e) => set("monthly_payment", e.target.value)} />
              </div>
            </div>
            <div style={GRID3}>
              <div>
                <label style={LABEL}>Remaining Term (years)</label>
                <input style={INPUT} type="number" step="0.5" value={form.remaining_years} onChange={(e) => set("remaining_years", e.target.value)} />
              </div>
              <div>
              ...
              <div>
                ...
                <input style={INPUT} value={form.lender || ""} onChange={(e) => set("lender", e.target.value)} />
              </div>
            </div>
            <div style={{ marginTop: 16, maxWidth: 200 }}>
              <label style={LABEL}>ERC %</label>
              <input style={INPUT} type="number" step="0.1" value={form.erc_percentage || ""} onChange={(e) => set("erc_percentage", e.target.value)} />
            </div>
          </div>
          ...
          <div style={CARD}>
            ...
            <div style={GRID3}>
              ...
              <div>
                ...
                <input style={INPUT} type="number" value={form.tenant_find_fee} onChange={(e) => set("tenant_find_fee", e.target.value)} />
              </div>
            </div>
          </div>
          ...
          <div style={CARD}>
            ...
            <div style={{ ...GRID3, marginBottom: 16 }}>
              <div>
                ...
                <input style={INPUT} type="number" value={form.buildings_insurance} onChange={(e) => set("buildings_insurance", e.target.value)} />
              </div>
              <div>
                <label style={LABEL}>Landlord Insurance</label>
                <input style={INPUT} type="number" value={form.landlord_insurance} onChange={(e) => set("landlord_insurance", e.target.value)} />
              </div>
              <div>
                <label style={LABEL}>Ground Rent</label>
                <input style={INPUT} type="number" value={form.ground_rent} onChange={(e) => set("ground_rent", e.target.value)} />
              </div>
            </div>
            <div style={GRID2}>
              <div>
                <label style={LABEL}>Service Charge</label>
                <input style={INPUT} type="number" value={form.service_charge} onChange={(e) => set("service_charge", e.target.value)} />
              </div>
              <div>
